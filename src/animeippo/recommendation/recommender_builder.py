...
def get_discovery_scorers():
    ...
    return [
        ...
        scoring.ClusterSimilarityScorer(weight=0.15),
        # Capture community taste relationships ("if you liked X, try Y")
        scoring.CollaborativeRecommendationScorer(weight=0.15),
        ...
        scoring.PopularityScorer(weight=0.05),
...
def get_default_categorizers(distance_metric="cosine", tag_lookup=None, genres=None):
    ...
    standard = [
        *debug,
        (categories.TopReleasedPicksCategory(exclusive=True), 3),
        (categories.ContinueWatchingCategory(), None),
        (categories.SimulcastsCategory(min_items=3), 40),
        (categories.TopUpcomingCategory(min_items=3), 35),
        (categories.PlanningCategory(), None),
        (categories.MovieNightCategory(exclusive=True), 3),
        (categories.YourTopPicksCategory(), 35),
        (categories.MostPopularCategory(), 20),
        (categories.HiddenGemsCategory(exclusive=True), 3),
